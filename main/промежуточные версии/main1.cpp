...
#define KUBE_POWER          GPIO_NUM_7                               // Порт включения куба
#define KEY_GPIO            GPIO_NUM_4                               // Порт кнопки
#define TOTAL_LEDS          (LEDS_PER_FACE * FACES_COUNT)

#define DEBOUNCE_TIME_MS    150                                      // время антидребезга
...
void cube_task(void *pvParam) {
    ...
    while(1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(10)); // опрос каждые 10 мс
    }
}

void initPins(){
    gpio_reset_pin(KUBE_POWER);
    gpio_set_direction(KUBE_POWER, GPIO_MODE_OUTPUT);

        gpio_config_t io_conf = {                                // Инициализация GPIO
        .pin_bit_mask = 1ULL << KEY_GPIO,
        .mode = GPIO_MODE_INPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,                        // подтяжка вверх
...
void KeyTask(void *pvParam){
    gpio_set_level(KUBE_POWER, true);                     // Включаем питание куба
    while (gpio_get_level(KEY_GPIO)){                     // Ждем отпускания кнопки при старте
        vTaskDelay(pdMS_TO_TICKS(50));
    }
    ESP_LOGI(TAG, "Key task started!");
    int last_state = 0;                                   // кнопка отпущена (лог.1)
    int stable_state = 0;
    TickType_t last_debounce_time = 0;
     while (1) {
        int reading = gpio_get_level(KEY_GPIO);
        if (reading != last_state) {                        // Проверка изменения состояния
            last_debounce_time = xTaskGetTickCount();
            last_state = reading;
        }
        if ((xTaskGetTickCount() - last_debounce_time) > pdMS_TO_TICKS(DEBOUNCE_TIME_MS)) {   // Проверяем, прошло ли время антидребезга
            if (reading != stable_state) {
                stable_state = reading;
                if (stable_state == 1){
                    ESP_LOGI(TAG, "Button pressed!");
                    gpio_set_level(KUBE_POWER, false);                                       // Выключаем питание куба
                }
                else{
...
extern "C" void app_main(void) {
    initPins();
    xTaskCreate(KeyTask, "key Task", 4096, NULL, 5, NULL);
